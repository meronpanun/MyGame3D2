...
SceneGameOver::SceneGameOver(int wave, int killCount, int score)
  ...
  m_japaneseFontHandle = CreateFontToHandle("HGPｺﾞｼｯｸE", (int)(20 * scale), 3,
  ...
  m_japaneseLargeFontHandle = CreateFontToHandle(
      "HGPｺﾞｼｯｸE", (int)(36 * scale), 3, DX_FONTTYPE_ANTIALIASING_EDGE_8X8);
  assert(m_japaneseLargeFontHandle != -1);
  m_japaneseButtonFontHandle = CreateFontToHandle(
      "HGPｺﾞｼｯｸE", (int)(24 * scale), 3, DX_FONTTYPE_ANTIALIASING_EDGE_8X8);
...
void SceneGameOver::Draw() {
  ...
  // 影
  DrawFormatStringToHandle((screenW - waveStrW) / 2 + shadowOffset,
                           textY + shadowOffset, 0x000000,
  ...
  // 影
  DrawFormatStringToHandle(m_layout.textLabelX + shadowOffset,
                           textY + shadowOffset, 0x000000,
                           m_japaneseLargeFontHandle, "倒した敵の数");
  DrawFormatStringToHandle(m_layout.textValueX + shadowOffset,
                           textY + shadowOffset, 0x000000,
                           m_japaneseLargeFontHandle, "%d", killCount);
  // 本体
  DrawFormatStringToHandle(m_layout.textLabelX, textY, 0xffffff,
                           m_japaneseLargeFontHandle, "倒した敵の数");
  DrawFormatStringToHandle(m_layout.textValueX, textY, 0xffffff,
                           m_japaneseLargeFontHandle, "%d", killCount);
  textY += textInterval;
  ...
  DrawFormatStringToHandle(m_layout.textLabelX + shadowOffset,
                           textY + shadowOffset, 0x000000,
                           m_japaneseLargeFontHandle, "スコア");
  DrawFormatStringToHandle(m_layout.textValueX + shadowOffset,
                           textY + shadowOffset, 0x000000,
                           m_japaneseLargeFontHandle, "%d",
  ...
  auto DrawButton = [&](int x1, int y1, int x2, int y2, const char *text,
    ...
    // テキスト
    int textWidth =
        GetDrawStringWidthToHandle(text, -1, m_japaneseButtonFontHandle);
    int textHeight =
        (int)(24 * Game::GetUIScale()); // フォントサイズに合わせて調整
    int textX = x1 + (x2 - x1 - textWidth) / 2;
    int textY = y1 + (y2 - y1 - textHeight) / 2;

    // 影
    DrawFormatStringToHandle(textX + 2, textY + 2, 0x000000,
                             m_japaneseButtonFontHandle, text);
    // 本体
    DrawFormatStringToHandle(textX, textY, 0xffffff, m_japaneseButtonFontHandle,
                             text);
  };

  // タイトルボタン
  bool isTitleHover =
      (mousePos.x >= m_layout.titleBtnX1 && mousePos.x <= m_layout.titleBtnX2 &&
       mousePos.y >= m_layout.titleBtnY1 && mousePos.y <= m_layout.titleBtnY2);
  DrawButton(m_layout.titleBtnX1, m_layout.titleBtnY1, m_layout.titleBtnX2,
             m_layout.titleBtnY2, "タイトルに戻る", isTitleHover);

  // リトライボタン
  bool isRetryHover =
      (mousePos.x >= m_layout.retryBtnX1 && mousePos.x <= m_layout.retryBtnX2 &&
       mousePos.y >= m_layout.retryBtnY1 && mousePos.y <= m_layout.retryBtnY2);
  DrawButton(m_layout.retryBtnX1, m_layout.retryBtnY1, m_layout.retryBtnX2,
             m_layout.retryBtnY2, "リトライ", isRetryHover);
}

void SceneGameOver::DrawGradientBox(int x1, int y1, int x2, int y2,
...
void SceneGameOver::UpdateLayout() {
  ...
  m_layout.resBgY =
      m_layout.imageDrawY + m_layout.imageDrawHeight + (int)(20 * scale);
