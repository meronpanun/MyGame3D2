...
SceneManager::SceneManager()
    : m_pTitle(nullptr), m_pOption(nullptr), m_pSceneMain(nullptr),
      m_pResult(nullptr), m_pGameOver(nullptr), m_pCurrentScene(nullptr),
      m_pNextScene(nullptr), m_pSceneToChange(nullptr),
      m_isExternalSceneChange(false), m_loadingDotCount(0),
      m_loadingAnimTimer(0), m_fadeState(FadeState::Idle), m_fadeAlpha(0),
      m_fadeSpeed(5) {}
    ...
    if (m_fadeAlpha >= 255) {
      ...
      if (m_pCurrentScene != nullptr) {
        ...
        if (m_pCurrentScene == m_pTitle)
          m_pTitle = nullptr;
        if (m_pCurrentScene == m_pSceneMain)
          m_pSceneMain = nullptr;
        if (m_pCurrentScene == m_pResult)
          m_pResult = nullptr;
        if (m_pCurrentScene == m_pOption)
          m_pOption = nullptr;
        if (m_pCurrentScene == m_pGameOver)
          m_pGameOver = nullptr;
