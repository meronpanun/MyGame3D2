...
constexpr VECTOR kHeadShotPositionOffset = {0.0f, 0.0f,
                                            0.0f}; // オフセットに変更
...
EnemyNormal::EnemyNormal()
    : m_headPosOffset{kHeadShotPositionOffset}, m_isTackleHit(false),
      ...
      m_currentAnimState(AnimState::Walk), m_attackEndDelayTimer(0),
...
void EnemyNormal::DeleteModel() { MV1DeleteModel(s_modelHandle); }
...
void EnemyNormal::Update(const EnemyUpdateContext &context) {
  ...
  if (m_pBodyCollider->IsIntersects(playerBodyCollider.get())) {
    ...
    float minDist = kBodyColliderRadius +
                    playerBodyCollider->GetRadius(); // 最小距離は両方の半径の和
...
// どこに当たったのか判定する
EnemyBase::HitPart EnemyNormal::CheckHitPart(const VECTOR &rayStart,
                                             const VECTOR &rayEnd,
                                             VECTOR &outHtPos,
...
// アイテムドロップ時のコールバック関数
void EnemyNormal::SetOnDropItemCallback(
