...
public:
    ...
    struct TackleInfo
    {
        VECTOR capA      = { 0, 0, 0 }; // タックル攻撃カプセルのA点
        VECTOR capB      = { 0, 0, 0 }; // タックル攻撃カプセルのB点
        float  radius    = 0.0f;        // タックル攻撃カプセルの半径
        float  damage    = 0.0f;        // タックルのダメージ量
        bool   isTackling = false;      // タックル中かどうか
        int    tackleId  = 0;           // タックルのID
    };
...
public:
    ...
    /// <summary>
    /// プレイヤーが射撃したかどうかを返す（呼び出し後フラグをリセット）
    /// </summary>
    /// <returns>射撃したならtrue</returns>
    bool HasShot();
    ...
    /// <summary>
    /// 敵に照準を合わせているかどうかを取得する
    /// </summary>
    /// <returns>照準を合わせているならtrue</returns>
...
private:
    ...
    // 位置・姿勢
    VECTOR m_pos;              // 論理的な位置（移動システムと同期）
    VECTOR m_modelPos;         // モデル描画に使う実際の位置
    VECTOR m_scale;            // モデルのスケール
    VECTOR m_gunSwayOffset;    // 銃の揺れオフセット
    VECTOR m_gunSwayRotOffset; // 銃の回転揺れオフセット

    // タイマー系
    float m_idleSwayTimer;     // 待機中の揺れタイマー
    ...
    bool m_isDamageHandledInThisFrame;   // このフレームで被弾演出（SE/振動）が処理されたか

    AttackType m_allowedAttackType = AttackType::None; // 許可されている攻撃タイプ（チュートリアル制限用）
};
