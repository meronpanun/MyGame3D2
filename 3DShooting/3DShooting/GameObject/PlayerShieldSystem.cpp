...
constexpr float kShieldReflectAngleCos =
    0.3f; // 反射を許可する入射角のコサイン閾値
constexpr float kShieldThrowCooldown = 1.0f; // シールド投げのクールタイム（秒）

// 盾投げ失敗アニメーション
constexpr float kShieldThrowFailedAnimDuration =
    0.2f; // 失敗アニメーションの持続時間（秒）
constexpr float kShieldThrowFailedAnimOffset =
    15.0f; // 失敗アニメーションの突き出し距離
} // namespace
...
PlayerShieldSystem::PlayerShieldSystem()
    : m_shieldModelHandle(-1), m_shieldImageHandle(-1),
      ...
      m_guardAnimTimer(0.0f), m_guardAnimDuration(kGuardAnimDuration),
      ...
      m_shieldThrowDistance(0.0f), m_shieldThrowSpeed(kShieldThrowSpeed),
      m_shieldThrowMaxRange(kShieldThrowMaxRange),
      m_shieldThrowDamage(kShieldThrowDamage), m_shieldThrowHitEnemyId(-1),
...
void PlayerShieldSystem::Draw(Camera *pCamera, const VECTOR &playerPos,
  ...
  VECTOR shieldCamTarget =
      VGet(totalCameraOffset.x * kShieldCamTargetFactor,
           totalCameraOffset.y * kShieldCamTargetFactor, 0);
  ...
  float easeProgress =
      1.0f - cosf(guardAnimProgress * DX_PI_F * 0.5f); // イージング

  // 待機位置とガード位置を定義
  VECTOR waitPos =
      VAdd(VGet(kShieldWaitX * scaleW, kShieldWaitY * scaleH, kShieldWaitZ),
           m_shieldSwayOffset);
  ...
  // 進行度に応じて回転を補間
  VECTOR currentRot =
      VAdd(waitRot, VScale(VSub(guardRot, waitRot), easeProgress));
  ...
  if (m_isShieldThrowFailedAnimating) {
    float animProgress =
        m_shieldThrowFailedAnimTimer / kShieldThrowFailedAnimDuration;
    // ease-out イージング（減速しながら戻る）
    float easedProgress = 1.0f - powf(1.0f - animProgress, 3.0f);
    // 前に突き出して戻る（0 → max → 0）
    float thrustOffset =
        sinf(easedProgress * DX_PI_F) * kShieldThrowFailedAnimOffset;
  ...
  MV1SetScale(m_shieldModelHandle,
              VGet(kShieldModelScale * scaleAvg, kShieldModelScale * scaleAvg,
                   kShieldModelScale * scaleAvg));
...
void PlayerShieldSystem::UpdateGuardEffect(Effect *pEffect, Camera *pCamera,
  ...
  if (m_isGuarding && (!m_wasGuarding || weaponSwitchJustFinished) &&
    ...
    // カメラ位置基準でエフェクト位置を計算
    VECTOR effectPos =
        VAdd(pCamera->GetPos(), VScale(forward, kGuardEffectOffsetZ));
...
bool PlayerShieldSystem::ThrowShield(Camera *pCamera, const VECTOR &playerPos) {
  ...
  m_shieldThrowDir =
      VNorm(cameraForward); // カメラの前方方向ベクトル（レティクル方向）
...
void PlayerShieldSystem::UpdateShieldThrow(
  ...
  if (m_shieldThrowState == ShieldThrowState::Throwing) {
    ...
    for (const auto &col : collisionData) {
      if (HitCheck_Sphere_Triangle(m_shieldThrowPos, kShieldThrowRadius, col.v1,
                                   col.v2, col.v3)) {
        // 法線の計算
  ...
  if (m_shieldThrowState == ShieldThrowState::Throwing ||
    ...
    // シールドの当たり判定（カプセル形状）
    VECTOR shieldCapA =
        VAdd(m_shieldThrowPos, VGet(0, -kShieldThrowHeight * 0.5f, 0));
    VECTOR shieldCapB =
        VAdd(m_shieldThrowPos, VGet(0, kShieldThrowHeight * 0.5f, 0));
    ...
    for (EnemyBase *enemy : enemyList) {
      ...
      VECTOR shieldCenter =
          VAdd(shieldCapA, VScale(VSub(shieldCapB, shieldCapA), 0.5f));
      VECTOR enemyCenter =
          VAdd(enemyCapA, VScale(VSub(enemyCapB, enemyCapA), 0.5f));
      VECTOR toEnemy = VSub(enemyCenter, shieldCenter);
      float dist = VSize(toEnemy);
      float minDist =
          kShieldThrowRadius + enemyRadius + kShieldThrowHeight * 0.5f;
...
void PlayerShieldSystem::DrawShieldThrow(Camera *pCamera,
  ...
  MATRIX rotAroundForward =
      MGetRotZ(m_shieldThrowRotationTimer); // Z軸周りの回転（横向きの状態）
  ...
  MV1SetScale(m_shieldModelHandle,
              VGet(kShieldModelScale, kShieldModelScale, kShieldModelScale));
  MV1DrawModel(m_shieldModelHandle);
}
