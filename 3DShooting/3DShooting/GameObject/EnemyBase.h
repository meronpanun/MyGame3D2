...
public:
  ...
  /// 当たり判定の部位
  /// </summary>
  enum class HitPart { None, Body, Head };
  ...
  virtual void
  SetOnDeathCallback(std::function<void(const VECTOR &)> callback) {
  ...
  virtual HitPart CheckHitPart(const VECTOR &rayStart, const VECTOR &rayEnd,
                               VECTOR &outHtPos, float &outHtDistSq) const {
    return HitPart::None;
  }
  ...
  virtual void SetOnDeathWithTypeCallback(
      std::function<void(const VECTOR &, AttackType)> cb) {
    m_onDeathWithTypeCallback = cb;
  }
