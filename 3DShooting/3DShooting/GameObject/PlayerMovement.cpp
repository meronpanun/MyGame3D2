...
constexpr float kLandingVelocityFactor =
    0.5f; // 着地時の衝撃（速度）による揺れの補正係数

// ダッシュ着地時のシェイク（画面全体の振動）
constexpr float kRunLandingShakeIntensity = 1.0f; // ベースシェイク強度
constexpr float kRunLandingShakeVelocityFactor =
    0.2f;                                   // 速度によるシェイク加算係数
...
void PlayerMovement::Update(
  ...
  CollisionResult preCollisionResult =
      Collision::CheckStageCollision(m_modelPos, kCapsuleHeight, kCapsuleRadius,
                                     kPlayerColliderYOffset, collisionData);
...
void PlayerMovement::UpdateFlightMode(float deltaTime, Camera *pCamera,
  ...
  if (keyState[KEY_INPUT_SPACE])
    m_modelPos.y += kFlightAscendSpeed * timeScale;
  if (keyState[KEY_INPUT_LSHIFT])
    m_modelPos.y -= kFlightDescendSpeed * timeScale;

  bool isAccelerating = (keyState[KEY_INPUT_LCONTROL] != 0);
  float speed =
      (isAccelerating ? m_runSpeed * kFlightAccelMultiplier : m_moveSpeed) *
      timeScale;
      ...
      if (VSize(moveDir) > 0.0f && pCamera) {
        ...
        if (inertiaSpeed > 0.1f) {
          ...
          VECTOR targetSideVelocity =
              VScale(camRight, dotRight * currentSpeed * kAirControlFactor);
          m_airSideControlVelocity.x +=
              (targetSideVelocity.x - m_airSideControlVelocity.x) *
              kAirAccelFactor * timeScale;
          ...
          float fwdProjDot =
              VDot(VScale(camFwd, dotFwd * currentSpeed * kAirControlFactor),
                   inertiaDir);
          if (fwdProjDot < 0.0f) {
  ...
  if (m_isJumping || !isOnGround) {
    ...
    if (m_modelPos.y <= kGroundY) {
      ...
      if (m_wasJumping && pCamera) {
        float swayPower =
            (m_isRunJumping ? kRunLandingSwayPower : kLandingSwayPower) +
            fabsf(lastJumpVelocity) * kLandingVelocityFactor;
        pCamera->ApplyLandingSway(swayPower);
        if (m_isRunJumping)
          pCamera->Shake(kRunLandingShakeIntensity +
                             fabsf(lastJumpVelocity) *
                                 kRunLandingShakeVelocityFactor,
                         kRunLandingShakeDuration);
      }
...
void PlayerMovement::ResolveCollisions(
  ...
  if (m_isRunMode && !m_isJumping && isMoving) {
    ...
    if (pushBackSq > 1.0f && !isOnSlope)
      m_isRunMode = false;
  }

  if (m_isGroundedOnStage && m_jumpVelocity < 0.0f) {
    if ((m_wasJumping || m_jumpVelocity < -5.0f) && pCamera) {
      float sway = (m_isRunJumping ? kRunLandingSwayPower : kLandingSwayPower) +
                   fabsf(m_jumpVelocity) * kLandingVelocityFactor;
