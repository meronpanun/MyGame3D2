...
PlayerWeaponManager::PlayerWeaponManager()
    : m_arHandle(-1), m_sgHandle(-1), m_ejectionPortFrame(-1), m_arAmmo(0),
      ...
      m_sgBulletPower(0.0f), m_shootCooldown(1.0f / kARShootRate),
      m_shootCooldownTimer(0.0f), m_arShootRate(kARShootRate),
...
void PlayerWeaponManager::Update(const UpdateContext &context) {
  ...
  m_pullBackOffset += (targetPullBack - m_pullBackOffset) *
                      (1.0f - powf(0.1f, deltaTime * 10.0f));

  // 画面外（カメラの後ろ）に消えないようにクランプ。
  // 初期Zオフセットが60.0f程度なので、80.0f程度を上限とする。
  if (m_pullBackOffset > 80.0f)
    m_pullBackOffset = 80.0f;
}
...
void PlayerWeaponManager::Draw3D(const DrawContext &context) {
  ...
  if (isTryingToGuard || isTackling) {
    ...
    if (prevHandle != -1) {
      float progress =
          (std::min)(m_weaponSwitchTimer / (m_weaponSwitchDuration / 2.0f),
                     1.0f);
      ...
      VECTOR cameraBasePos =
          VAdd(playerPos, VTransform(pCamera->GetOffset(), camRotYaw));
      ...
      float checkDistance =
          (previousWeaponType == WeaponType::AssaultRifle) ? 160.0f : 180.0f;
      ...
      finalPos =
          VSub(finalPos, VScale(camRight, pullProgress * 60.0f)); // 左に寄せる
      finalPos =
          VAdd(finalPos, VScale(camUp, pullProgress * 20.0f)); // 上に寄せる
    ...
    // 新しい武器を下から出す
    auto [currentHandle, currentOffset] =
        GetWeaponTransform(m_currentWeaponType);
    if (currentHandle != -1) {
      float progress = (std::max)(0.0f, (m_weaponSwitchTimer -
                                         (m_weaponSwitchDuration / 2.0f)) /
                                            (m_weaponSwitchDuration / 2.0f));
      float easeOut = 1.0f - powf(1.0f - progress, 3.0f);
      ...
      float checkDistance =
          (m_currentWeaponType == WeaponType::AssaultRifle) ? 160.0f : 180.0f;
      float pullProgress = (std::min)(1.0f, m_pullBackOffset / checkDistance);
    ...
    int otherHandle = (m_currentWeaponType == WeaponType::AssaultRifle)
                          ? m_sgHandle
                          : m_arHandle;
    ...
    if (currentHandle != -1) {
      ...
      float gunOffsetY =
          -200.0f * (1.0f - cosf(guardAnimProgress * DX_PI_F * 0.5f));
      ...
      float checkDistance =
          (m_currentWeaponType == WeaponType::AssaultRifle) ? 220.0f : 240.0f;
      float pullProgress = (std::min)(1.0f, m_pullBackOffset / checkDistance);
...
void PlayerWeaponManager::SwitchWeapon(WeaponType weaponType) {
  ...
  m_prevWeaponHadLowAmmo =
      (prevAmmo > 0 && prevAmmo <= kLowAmmoThreshold) && !m_isInfiniteAmmo;
...
void PlayerWeaponManager::Shoot(std::vector<Bullet> &bullets,
  ...
  case WeaponType::AssaultRifle:
    ...
    currentMuzzleFlashOffset =
        VGet(kARMuzzleFlashEffectOffsetX, kARMuzzleFlashEffectOffsetY,
             kARMuzzleFlashEffectOffsetZ);
  ...
  case WeaponType::Shotgun:
    ...
    currentMuzzleFlashOffset =
        VGet(kSGMuzzleFlashEffectOffsetX, kSGMuzzleFlashEffectOffsetY,
             kSGMuzzleFlashEffectOffsetZ);
  ...
  // SEを再生
  int freq = static_cast<int>(kBaseSoundFrequency * Game::GetTimeScale());
  if (freq < 100)
    freq = 100;
...
void PlayerWeaponManager::AddARAmmo(int value) {
  m_arAmmo += value;
  if (m_arAmmo > m_arMaxAmmo)
    m_arAmmo = m_arMaxAmmo;
}

void PlayerWeaponManager::AddSGAmmo(int value) {
  m_sgAmmo += value;
  if (m_sgAmmo > m_sgMaxAmmo)
    m_sgAmmo = m_sgMaxAmmo;
}
...
VECTOR PlayerWeaponManager::GetGunPos(const VECTOR &playerPos,
  ...
  // カメラのボビング（揺れ）を含まない基準位置を計算
  VECTOR cameraBasePos =
      VAdd(playerPos, VTransform(pCamera->GetOffset(), rotYaw));
  ...
  gunMuzzlePos =
      VSub(gunMuzzlePos, VScale(camRight, pullProgress * 60.0f)); // 左に寄せる
  gunMuzzlePos =
      VAdd(gunMuzzlePos, VScale(camUp, pullProgress * 20.0f)); // 上に寄せる
...
void PlayerWeaponManager::UpdateSGAnimation(AnimationManager *pAnimManager,
  ...
  if (m_isSGAnimPlaying) {
    ...
    if (pAnimManager) {
      ...
      // アニメーションが終了したかチェック
      float totalTime = pAnimManager->GetAnimationTotalTime(
          m_sgHandle,
          "Armature.001|Armature.001|lever action_FIRE|Baked frames");
      if (totalTime > 0 && m_sgAnimTime >= totalTime) {
        m_isSGAnimPlaying = false;
        int attachIndex =
            pAnimManager->GetCurrentAttachedAnimHandle(m_sgHandle);
        if (attachIndex != -1) {
...
float PlayerWeaponManager::CalculatePullBackOffset(
  ...
  for (const auto &poly : collisionData) {
    float t = 0.0f;
    if (Collision::IntersectRayTriangle(rayStart,
                                        VScale(camForward, checkDistance),
                                        poly.v1, poly.v2, poly.v3, t)) {
      if (t >= 0.0f && t < minT) {
        minT = t;
        hit = true;
      }
  ...
  for (const auto &enemy : enemyList) {
    ...
    if (enemy->CheckHitPart(rayStart, rayEnd, hitPos, hitDistSq) !=
        EnemyBase::HitPart::None) {
