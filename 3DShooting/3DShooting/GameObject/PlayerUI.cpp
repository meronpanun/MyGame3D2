...
void PlayerUI::Draw(bool isDead, bool isGuarding, EnemyBase* lockedOnEnemy, bool isTargetAvailable,
...
void PlayerUI::DrawHPBar(float health, float healthBarAnim, float maxHealth)
{
	...
	// HP数値
	DrawFormatStringToHandle(barX + kHpTextOffsetX, barY + kHpTextOffsetY, kColorWhite, m_hpFontHandle, "%.0f", healthBarAnim);
}
...
void PlayerUI::DrawWarningUI(bool isLowHealth, float lowHealthBlinkTimer, const PlayerWeaponManager& weaponManager)
{
	int screenW, screenH;
	GetScreenState(&screenW, &screenH, NULL);
...
void PlayerUI::DrawGuardText(bool isGuarding, EnemyBase* lockedOnEnemy, bool isTargetAvailable)
{
