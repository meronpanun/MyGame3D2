...
EnemyAcid::EnemyAcid()
    : m_headPosOffset{kHeadShotPositionOffset}, m_animTime(0.0f),
      m_currentAnimState(AnimState::Walk), m_onDropItem(nullptr),
...
void EnemyAcid::DeleteModel() { MV1DeleteModel(s_modelHandle); }
...
void EnemyAcid::ChangeAnimation(AnimState newAnimState, bool loop) {
  ...
  if (m_currentAnimState == newAnimState) {
    if (newAnimState == AnimState::Attack || newAnimState == AnimState::Back) {
      m_animationManager.PlayAnimation(
          m_modelHandle,
      ...
      if (newAnimState == AnimState::Attack)
        m_hasAttacked = false;
      if (newAnimState == AnimState::Back)
        m_backAnimCount = 0;
    }
    return;
  }
...
void EnemyAcid::UpdateAcidBalls(const EnemyUpdateContext &context) {
  ...
  for (auto &ball : m_acidBalls) {
    ...
    if (ball.effectHandle != -1) {
      SetPosPlayingEffekseer3DEffect(ball.effectHandle, ball.pos.x, ball.pos.y,
                                     ball.pos.z);
    }
...
void EnemyAcid::UpdateCollision(const EnemyUpdateContext &context) {
  ...
  VECTOR hipsPos =
      (hipsIndex != -1) ? MV1GetFramePosition(m_modelHandle, hipsIndex) : m_pos;

  VECTOR bodySegmentP1 =
      VAdd(hipsPos, VGet(0, ::kBodyColliderHeight * 0.5f, 0));
  VECTOR bodySegmentP2 =
      VAdd(hipsPos, VGet(0, -::kBodyColliderHeight * 0.5f, 0));
...
void EnemyAcid::UpdateState(const EnemyUpdateContext &context) {
  ...
  if (m_attackEndDelayTimer == 0) {
    ...
    if (m_currentAnimState == AnimState::Back) {
      animTotal = m_animationManager.GetAnimationTotalTime(m_modelHandle,
                                                           kBackAnimName);
...
void EnemyAcid::DrawCollisionDebug() const {
  ...
  if (m_pBodyCollider) {
    DebugUtil::DrawCapsule(m_pBodyCollider->GetSegmentA(),
                           ...
                           m_pBodyCollider->GetRadius(), 16, 0xff00ff);
  }
  if (m_pHeadCollider) {
    DebugUtil::DrawSphere(m_pHeadCollider->GetCenter(),
                          m_pHeadCollider->GetRadius(), 16, 0xffff00);
  }
  if (m_pAttackRangeCollider) {
    DebugUtil::DrawSphere(m_pAttackRangeCollider->GetCenter(),
                          m_pAttackRangeCollider->GetRadius(), 16, 0x00ffff);
  }
  if (m_shouldDrawParryCollider) {
    DebugUtil::DrawCapsule(m_debugParryCapA, m_debugParryCapB,
                           m_debugParryRadius, 16, 0x0000ff);
  }
#endif
}

// どこに当たったのか判定する
EnemyBase::HitPart EnemyAcid::CheckHitPart(const VECTOR &rayStart,
                                           const VECTOR &rayEnd,
                                           VECTOR &outHtPos,
