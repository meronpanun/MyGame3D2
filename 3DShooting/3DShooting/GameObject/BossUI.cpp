...
{
	// ボスHPバー関連
	constexpr int   kBossHpBarWidth      = 600;
	constexpr int   kBossHpBarHeight     = 16;
	constexpr int   kBossHpBarY          = 110;
	constexpr int   kBossHpTextY         = 80;
...
BossUI::BossUI() :
	...
	m_fontHandle(-1)
{
	m_fontHandle = CreateFontToHandle("ＭＳ ゴシック", 24, 3, DX_FONTTYPE_ANTIALIASING_EDGE_8X8);
}
...
void BossUI::DrawBossHPBar(float hp, float maxHp)
{
	...
	// 背景
	DrawBox(barX, barY, barX + kBossHpBarWidth, barY + kBossHpBarHeight, kColorHpBarBg, true);

	// アニメーションバー（ダメージ演出用）
	if (m_healthBarAnim > hp)
	{
		int animW = static_cast<int>(kBossHpBarWidth * animRate);
		DrawBox(barX, barY, barX + animW, barY + kBossHpBarHeight, kColorHpBarDamage, true);
	}

	// HPバー本体
	int hpW = static_cast<int>(kBossHpBarWidth * hpRate);
	DrawBox(barX, barY, barX + hpW, barY + kBossHpBarHeight, kColorHpBarFill, true);
