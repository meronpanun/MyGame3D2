...
WaveUI::WaveUI()
    : m_waveImageAnimTimer(0), m_waveImageAnimDuration(45),
      m_waveImageAnimHoldDuration(30), m_waveImageAnimInitialHoldDuration(30),
...
void WaveUI::DrawWaveUI(int currentWave, bool isWaveActive,
  ...
  // ウェーブ中は常に画像を表示
  if (!isAllWavesCompleted && currentWave >= 1 && currentWave <= 5 &&
      (m_isWaveImageAnimating || isWaveActive)) {
...
void WaveUI::DrawDebugInfo(int currentWave, int aliveEnemyCount,
  ...
  // DrawBox(0, 0, 300, 150, 0x000000, TRUE);
