...
namespace
{
	...
	constexpr int   kMessageTitleFontSize	= 24;
	constexpr int   kMessageDetailFontSize  = 18;
	constexpr int   kMessageOffsetY			= 150;
	constexpr int   kMessageLineSpacing		= 5;   // メッセージの行間
	constexpr int   kMessageTimeBarHeight   = 4;   // タイムバーの高さ
	constexpr int   kMessageTimeBarPaddingY =16;   // タイムバーの上下のパディング
}
	    ...
	    m_isStepCompleted(false)
	{
		...
		m_messageDetailFontHandle = CreateFontToHandle("HGPｺﾞｼｯｸE", kMessageDetailFontSize, 3, DX_FONTTYPE_ANTIALIASING_EDGE_8X8);
...
void TutorialManager::DrawMessages(int screenW, int screenH)
{
    ...
    for (const auto& msg : m_messages)
    {
        ...
        // 幅の計算
        int title_width = GetDrawStringWidthToHandle(msg.title.c_str(), msg.title.length(), m_japaneseLargeFontHandle);
        int max_detail_width = 0;
        for (const auto& line : detail_lines) 
		{
            int line_width = GetDrawStringWidthToHandle(line.c_str(), line.length(), m_messageDetailFontHandle);
            if (line_width > max_detail_width) 
			{
                max_detail_width = line_width;
            }
        }
        int box_width = (std::max)(title_width, max_detail_width) + kBoxPaddingX * 2;
        ...
        // タイトル
        DrawStringToHandle(content_x, current_y, msg.title.c_str(), 0xFFFFFF, m_japaneseLargeFontHandle);
        ...
        if (msg.state == UIState::OnScreen) 
		{
            ...
            DrawBox(content_x, current_y, content_x + bar_width, current_y + kMessageTimeBarHeight, 0xFF0000, true);
        }
        ...
        for (const auto& line : detail_lines) 
		{
            ...
            if (pos1 != std::string::npos) 
			{
                ...
                DrawStringToHandle(current_x, current_y, part1.c_str(), 0xFFFFFF, m_messageDetailFontHandle);
                current_x += GetDrawStringWidthToHandle(part1.c_str(), part1.length(), m_messageDetailFontHandle);

                DrawStringToHandle(current_x, current_y, keyword1.c_str(), 0xFFD700, m_messageDetailFontHandle);
                current_x += GetDrawStringWidthToHandle(keyword1.c_str(), keyword1.length(), m_messageDetailFontHandle);

                DrawStringToHandle(current_x, current_y, part3.c_str(), 0xFFFFFF, m_messageDetailFontHandle);
            }
            else if (pos2 != std::string::npos) 
			{
                ...
                DrawStringToHandle(current_x, current_y, keyword2.c_str(), 0xFFD700, m_messageDetailFontHandle);
            }
            else 
			{
                DrawStringToHandle(content_x, current_y, line.c_str(), 0xFFFFFF, m_messageDetailFontHandle);
            }
